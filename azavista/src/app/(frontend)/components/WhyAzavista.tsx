...
export default function WhyAzavistaBlock() {
  const [data, setData] = useState<WhyAzavistaData | null>(null);

  useEffect(() => {
    async function fetchData() {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_PAYLOAD_URL}/api/why-azavista?limit=1&depth=2`
        );
        const json = await res.json();
        if (json.docs?.length > 0) {
          setData(json.docs[0] as WhyAzavistaData);
        }
      } catch (err) {
        console.error("Failed to fetch Why Azavista data:", err);
      }
    }
    fetchData();
  }, []);
