...
export default async function WhyAzavistaBlock() {
  ...
  const data = docs?.[0] as WhyAzavistaData | undefined
  if (!data) return null
  ...
  return (
    <section className="py-16 bg-white">
      <div className="max-w-7xl mx-auto px-4 text-center">
        ...
        {data.title && (
          <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-4">{data.title}</h2>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {data.features?.map((feature, i) => {
            ...
            return (
              <div key={i} className="bg-white rounded-lg p-6 flex flex-col items-start text-left">
                {imageUrl ? (
                  <Image
                    src={imageUrl}
                ...
                <p className="text-blue-600 font-semibold text-sm">{feature.headingBlue}</p>
                <p className="text-gray-900 font-bold text-lg mt-1">{feature.headingBlack}</p>
